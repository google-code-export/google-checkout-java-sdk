...
<html>
  <head>
    <title>Add Merchant Order Number</title>
  </head>
  <body>
  ...
  function postIt(v) {
    ...
    if (button == null || button.equals("")) {
      amonRequest = new AddMerchantOrderNumberRequest(mi);
      ...
      amonRequest = (AddMerchantOrderNumberRequest) session.getAttribute("amonRequest");
      
      orderNumber = request.getParameter("orderNumber");
      merchantOrderNumber = request.getParameter("merchantordernumber");
      
      amonRequest.setGoogleOrderNo(orderNumber);
      amonRequest.setMerchantOrderNo(merchantOrderNumber);
      
      prettyXml = amonRequest.getXmlPretty();
      responseXml = "";
    } else if (button.equals("Submit")) {
      amonRequest = (AddMerchantOrderNumberRequest) session.getAttribute("amonRequest");
      CheckoutResponse res = amonRequest.send();
      
      prettyXml = amonRequest.getXmlPretty();
      responseXml = res.getXmlPretty();
      
      GoogleOrder order = GoogleOrder.findOrCreate(mi.getMerchantId(), orderNumber);
      order.addOutgoingMessage(new Date(), "add-merchant-order-number", prettyXml, responseXml);
    }
    %>
    <p>
    <h1>Add Merchant Order Number</h1>
    <form action="addmerchantordernumber.jsp" method="POST">
          ...
          <td>
            <table>
              <tr>
                ...
                <td>Merchant Order Number:</td><td><input type="text" name="merchantordernumber" value="<%=merchantOrderNumber %>"/></td>
              </tr>
            </table>
          </td>
        </tr>
