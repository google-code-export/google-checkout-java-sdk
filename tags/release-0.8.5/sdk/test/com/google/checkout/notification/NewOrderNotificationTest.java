...
public class NewOrderNotificationTest extends TestCase {
  ...
  public void setUp() {
    newOrderNotificationMessage = TestUtils
      .readMessage("/resources/new-order-notification-sample.xml");
    
    try {
      Document doc = 
        Utils.newDocumentFromString(newOrderNotificationMessage);
  ...
  public void testGetShoppingCart() {
    ...
    try {
      ...
      assertEquals("Mon Dec 31 23:59:59 PST 2007", cartExpDate.toString());
  ...
  public void testGetFulfillmentOrderState() {
    FulfillmentOrderState state = newOrderNotification.getFulfillmentOrderState();
  ...
  private void verfifyShoppingCartItems(Collection items) {
    ...
    assertEquals("One pack of nutritious dried food for emergencies.", 
    ...
    assertEquals("This portable MP3 player stores 500 songs.", 
  ...
  private void verifyMerchantCodes(Collection merchantCodes) {
    ...
    assertEquals("You saved $5.00 for your first visit!", 
      couponAdjustment.getMessage());
    
    GiftCertificateAdjustment giftAdjustment = (GiftCertificateAdjustment)it.next();
    ...
    assertEquals("You saved $10.00 with this gift certificate!", 
      giftAdjustment.getMessage());
